module.exports = app => {
  ...
  router.get('/news/list', async(req, res) => {
    ...
    const cats = await Category.aggregate([
      ...
      {
        $lookup: {
          from: 'articles', //关联集合名称，集合集合名称默认为模型名称复数小写
          localField: '_id',
          foreignField: 'categories',
          as: 'newsList' // 把查到的表起个名字
        }
      },
      // 3.定义查询的数据个数
      {
  ...
  router.get('/heroes/:id', async(req, res) => {
    const data = await Hero.findById(req.params.id).populate('categories items1 items2 partner.hero').lean()
    res.send(data)
  })
  // 调用幻灯片广告接口
